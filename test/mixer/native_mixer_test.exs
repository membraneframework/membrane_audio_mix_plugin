defmodule Membrane.AudioMixer.NativeMixerTest do
  @moduledoc false

  use ExUnit.Case, async: true

  import Membrane.AudioMixer.NativeAdder
  ...
  defp test_for_stream_format(stream_format_contents, buffers, reference) do
    ...
    |> Enum.each(fn stream_format ->
      ...
      {flushed, _state} = flush(state)
      assert result <> flushed == reference
    end)
  end
