...
#     list
#       |> with_index
#       |> into(%{}, fn {{q, d}, i} -> {i, %{queue: if q == <<>> do IOQueue.new else IOQueue.new(q) end, to_drop: d, first_play: false}} end)
#   end
...
#       defp binarize_state_queues %{sink_data: sink_data} = state do
#         %{state | sink_data: sink_data |> map(fn {s, %{queue: q} = sd} -> {s, %{sd | queue: q |> IOQueue.to_binary}} end)}
#       end
...
#             :ok, %{state | sink_data: simple_sink_data |> Map.update!(0, &%{&1 | queue: IOQueue.new(<<1,2,3,4,5,6>>) |> IOQueue.push(<<9,8,7>>) })}
#           }
#         end
#       end
...
#               state | sink_data: empty_sink_data |> Map.update!(1, &%{&1 | queue: IOQueue.new <<8,7>>})
#             }}
#         end
#       end
#     end
...
#       defp handle_other_ok_result [payload: payload, remaining_samples_cnt: remaining_samples_cnt, state: state] do
#         {
#           :ok,
...
#         sink_data |> Map.values |> map(fn %{queue: q} -> q |> IOQueue.to_iolist end)
#       end
...
#           expect(described_module.handle_other :tick, state).to eq handle_other_ok_result [
#             payload: sink_data |> Map.delete(1) |> queues,
#             remaining_samples_cnt: 0,
...
#         let :sink_data, do: simple_sink_data |> Map.update!(1, &%{&1 | queue: &1.queue |> IOQueue.push(<<80, 81, 82>>), first_play: true})
#         it "should add their buffer to payload, excess to queue, leave to_drop unchanged and set first_play to false" do
#           expect(described_module.handle_other :tick, state).to eq handle_other_ok_result [
#             payload: sink_data |> Map.update!(1, &%{&1 | queue: simple_sink_data[1].queue}) |> queues,
#             remaining_samples_cnt: 0,
#             state: %{state | sink_data: empty_sink_data |> Map.update!(1, &%{&1 | queue: IOQueue.new(<<80, 81, 82>>)})}
#           ]
#         end
#       end
...
#           let :sink_data, do: simple_sink_data |> Map.update!(1, &%{&1 | queue: IOQueue.new <<1,2,3,4,5,6,7,8,9>>})
#           it "should forward queue and store excess in the new queue" do
#             expect(described_module.handle_other :tick, state).to eq handle_other_ok_result [
#               payload: simple_sink_data |> Map.update!(1, &%{&1 | queue: IOQueue.new <<1,2,3,4,5,6>>}) |> queues,
#               remaining_samples_cnt: 0,
#               state: %{state | sink_data: empty_sink_data |> Map.update!(1, &%{&1 | queue: IOQueue.new <<7,8,9>>})}
#             ]
#           end
#         end
#         context "and one of them lacks data" do
#           let :sink_data, do: simple_sink_data |> Map.update!(1, &%{&1 | queue: IOQueue.new(<<1,2,3>>), to_drop: 1})
...
#             let :sink_data, do: empty_sink_data |> Map.update!(1, &%{&1 | queue: IOQueue.new <<1,2,3,4,5>>})
