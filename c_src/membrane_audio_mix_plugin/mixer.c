...
UNIFEX_TERM init(UnifexEnv *env, int32_t channels, uint32_t sample_format,
  ...
  state->sample_max = stream_format_audio_raw_sample_max(&state->stream_format);
  state->sample_min = stream_format_audio_raw_sample_min(&state->stream_format);
  state->sample_size = stream_format_audio_raw_sample_byte_size(&state->stream_format);
  state->queue = NULL;
...
void get_values(UnifexPayload **buffers, uint32_t buffers_length,
  ...
  for (uint32_t chunk_start = 0, i = 0; i < values_length;
    ...
    for (uint32_t j = 0; j < buffers_length; ++j) {
      ...
      values[i] += stream_format_audio_raw_sample_to_value(sample, &state->stream_format);
    }
  }
}
...
void scale_to_samples(uint8_t *samples, int64_t *values, uint32_t values_length,
  ...
  for (uint32_t i = 0; i < state->queue_length;
    ...
    stream_format_audio_raw_value_to_sample(state->queue[i], current_sample,
                                   &state->stream_format);
  }

  for (uint32_t i = 0; i < values_length;
       ++i, current_sample += state->sample_size) {
    stream_format_audio_raw_value_to_sample(values[i], current_sample, &state->stream_format);
  }
